...
from zepben.evolve import connect_async, NetworkConsumerClient, NetworkService, ConnectivityNode, \
    Junction, EnergyConsumer, Switch, Connector, PowerTransformer, PowerTransformerEnd, Conductor, Feeder
...
feeder = list(network_service.objects(Feeder))[0]
# NetworkService Inventory
network_service_inventory(network_service)

# Create PandaPower model
pp_net = pp.create_empty_network()
EvolveToPandaPowerMap(network_service=network_service, pp_net=pp_net, feeder_mrid=feeder.mrid)
print(pp_net)
pp.runpp(pp_net)
print(pp_net.res_bus)
print(pp_net.res_trafo)
print(pp_net.res_line)
print(pp_net.res_load)
print(pp_net.ext_grid)
pp.to_sqlite(net=pp_net, filename='example_pp_net.sqlite', include_results=True)
