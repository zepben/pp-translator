...
def create_bus(service, net):
    ...
    for Term in service.objects(ConnectivityNode):
        ...
        #Visitados=[]
        #if Recorrer(Term,Visitados)!=None:
        ...
        if (len(Transformer._terminals) == 2):
            ...
            try:
                ...
                if net.bus.at[NODE1,"vn_kv"]>net.bus.at[NODE2,"vn_kv"]:
                    ...
                    lv_bus=NODE1
                #else:
                    #hv_bus = get_index(conn_to_junction(Transformer._terminals[1].connectivity_node).mrid, a)
                    #lv_bus = get_index(conn_to_junction(Transformer._terminals[0].connectivity_node).mrid, a)
                ...
                i0_percent = 1
                shift_degree = 0
                pp.create_transformer_from_parameters(net, name=Transformer.mrid, hv_bus=hv_bus, lv_bus=lv_bus, sn_mva=0.005,
...
async def main(argv):
    ...
    async with connect_async(host=host, rpc_port=rpc_port) as channel:
        ...
        #Descomentariar la siguiete linea
