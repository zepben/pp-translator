...
async def main():
    ...
    async with connect_async(host=args.server, rpc_port=args.rpc_port) as channel:
        ...
        for idx in term_resultsX.index:
            term_resultsX.at[idx, 'SvVoltage'] = {
                "angle": term_resultsX.at[idx, 'va_degree'],
                "v": term_resultsX.at[idx, 'vn_kv'],
                "TopologicalNode_mRID": term_resultsX.at[idx, 'TopologicalNode_mRID']
            }
            term_resultsX.at[idx, 'SvPowerFlow'] = {
                "p": term_resultsX.at[idx, 'p'], "q": term_resultsX.at[idx, 'q'],
...
def _get_current_utilisation(acls: AcLineSegment, dataframe: DataFrame):
    ...
    if len(ps) > 0:
        avg_p = mean(ps)
        base_voltage = acls.base_voltage.nominal_voltage * 1000 if acls.base_voltage is not None else None
        rated_current = acls.wire_info.rated_current if acls.wire_info is not None else None
        if base_voltage is None or rated_current is None:
            print(f"AcLineSegment ${acls.mrid} has no base voltage or rated current")
            return None
        # PTI_CODE: 3 Core 95mm Al OH 11kV XLPE ABC - Country Energy Cable List - Updated 16 / 9 / 2008
        if rated_current == 0:
            rated_current = 195
        return round(((avg_p / base_voltage) / rated_current), 2)
    else:
        print(f"AcLineSegment {acls.mrid} has no p values")
        return None


def _get_avg_p(acls: AcLineSegment, dataframe: DataFrame):
    ps = list(dataframe.loc[dataframe['ConductingEquipment_mRID'] == acls.mrid, "p"])
