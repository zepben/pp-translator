...
from zepben.evolve import EquipmentContainer,AcLineSegment,Junction,EnergyConsumer, Breaker,PowerTransformer,Fuse,Recloser
...
async def main():
    async with connect_async(host="localhost", rpc_port=50052) as channel:
        ...
        result = (await client.get_feeder(service, 'SimpleNetwork')).throw_on_error()

        print(service.get('SimpleNetwork'))
        ...
        for Equip in service.objects(AcLineSegment):
            ...
            pp.create_line_from_parameters(net,name=Equip.mrid,from_bus=from_bus,to_bus=to_bus,length_km=length_km,r_ohm_per_km=r_ohm_per_km,x_ohm_per_km=x_ohm_per_km,c_nf_per_km=c_nf_per_km,max_i_ka=max_i_ka,r0_ohm_per_km=r0_ohm_per_km,x0_ohm_per_km=x0_ohm_per_km,c0_nf_per_km=c0_nf_per_km)
        print("Las líneas en PandaPower son:")
        print(net.line)
        #pp.add_zero_impedance_parameters(net)
        pp.runpp_3ph(net)
        #pp.runpp(net)
        print("Los resultados de simulaciones son:")
        #print(net.res_bus)
        pd.set_option('display.max_columns', None)
