...
class TransformerPp(object):
    ...
    def __init__(self, transformer):
        if len(transformer._terminals) == 2:
            ...
            self.vn_hv_kv = transformer._power_transformer_ends[0].ratedU
            self.vn_lv_kv = transformer._power_transformer_ends[1].ratedU

            # Extract parameters
            s = transformer._power_transformer_ends[0].rated_s
            ...
            self.vk_percent = sqrt(r**2+x**2)*s/(1.732*self.vn_hv_kv**2)
            self.vkr_percent = (r*s)/(1.732*self.vn_hv_kv**2)
            self.pfe_kw = (g*self.vn_hv_kv**2)/1000
            self.i0_percent = (sqrt(b**2+g**2)*1.732*self.vn_hv_kv**2)/s
            self.vk0_percent = sqrt(r0**2+x0**2)*s/(1.732*self.vn_hv_kv**2)
            self.vkr0_percent = (r0*s)/(1.732*self.vn_hv_kv**2)
            self.mag0_rx = b0/g0
            self.mag0_percent = 1/sqrt(b0**2+g0**2)
